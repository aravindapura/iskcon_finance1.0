...
const WarehousePage = () => {
  ...
  const handleBookSubmit = useCallback(
    (event: FormEvent<HTMLFormElement>) => {
      ...
      const languageValue = String(formData.get("bookLanguage") ?? "");
      ...
      const purchaseValue = Number(formData.get("bookPurchasePrice"));
      const saleValue = Number(formData.get("bookSalePrice"));
      const debtValue = Number(formData.get("bookDebt"));
      ...
      if (!selectedLanguage) {
        setBookFormError("Выберите язык из списка");
        return;
      }

      if (!Number.isFinite(quantityValue) || quantityValue <= 0) {
        setBookFormError("Введите корректное количество экземпляров");
        return;
      }

      const purchasePrice = normalizeMoney(purchaseValue);
      const salePrice = normalizeMoney(saleValue);
      const debt = normalizeMoney(debtValue);
  ...
  const handleBookSaleSubmit = useCallback(
    (event: FormEvent<HTMLFormElement>) => {
      ...
      const bookId = String(formData.get("soldBookId") ?? "").trim();
      const quantityString = String(formData.get("soldBookQuantity") ?? "").trim();
      const salePriceString = String(formData.get("soldBookSalePrice") ?? "").trim();
    ...
    <PageContainer activeTab="warehouse">
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-8">
        <div className="rounded-3xl border border-slate-200 bg-white/70 px-6 py-6 text-center shadow-sm backdrop-blur dark:border-slate-700 dark:bg-slate-900/70">
          <h1 className="text-2xl font-semibold text-slate-900 dark:text-white">Модуль инвентаря и книг</h1>
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
            Выберите нужную вкладку, чтобы управлять записями, библиотекой и статусом ценностей.
          </p>
        </div>
        ...
        {activeTab === "inventory" ? (
          ...
          <div className="space-y-6">
            ...
            >
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Добавление книги</h2>
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                  Заполните карточку, чтобы зафиксировать новую книгу в библиотечном фонде.
                </p>
              </div>

              <div className="grid gap-5 md:grid-cols-2">
                <label className="flex flex-col gap-2 md:col-span-2">
                  <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">Название</span>
                  <input
                    type="text"
                    name="bookTitle"
                ...
                <label className="flex flex-col gap-2">
                  <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">Язык</span>
                  <select
                    name="bookLanguage"
                    required
                    defaultValue=""
                ...
                <label className="flex flex-col gap-2">
                  <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">Количество</span>
                  <input
                    type="number"
                    name="bookQuantity"
                    min="1"
                ...
                <label className="flex flex-col gap-2">
                  <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">Цена закупки</span>
                  <input
                    type="number"
                ...
                <label className="flex flex-col gap-2">
                  <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">Цена реализации</span>
                  <input
                    type="number"
                    name="bookSalePrice"
                    min="0"
                ...
                <label className="flex flex-col gap-2">
                  <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">Долг за книги</span>
                  <input
                    type="number"
                    name="bookDebt"
                    min="0"
              ...
              {bookFormError ? (
                <p className="mt-4 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700 dark:border-rose-500/60 dark:bg-rose-500/10 dark:text-rose-200">
                  {bookFormError}
                </p>
              ) : bookFormSuccess ? (
                <p className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700 dark:border-emerald-500/60 dark:bg-emerald-500/10 dark:text-emerald-200">
                  {bookFormSuccess}
                </p>
              ) : null}
              ...
              <div className="mt-6 flex items-center justify-between gap-3">
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center rounded-xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-slate-500 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:bg-slate-800/80"
                >
            ...
            <form
              className="rounded-3xl border border-slate-200 bg-white/80 p-6 shadow-sm backdrop-blur dark:border-slate-700 dark:bg-slate-900/80"
              onSubmit={handleBookSaleSubmit}
            >
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Фиксация продажи</h2>
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                  Отметьте реализованные экземпляры — остатки пересчитаем автоматически.
                </p>
              </div>

              <div className="grid gap-5 md:grid-cols-2">
                <label className="flex flex-col gap-2 md:col-span-2">
                  <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">Книга из каталога</span>
                  <select
                    name="soldBookId"
                    value={saleBookId}
                    onChange={(event) => setSaleBookId(event.target.value)}
                    required
                    disabled={!availableBooksForSale.length || isBookSaleSubmitting}
                    className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-base text-slate-900 shadow-sm transition focus:border-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-400 disabled:cursor-not-allowed disabled:bg-slate-100 dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:disabled:border-slate-700/60 dark:disabled:bg-slate-800/60"
                  >
                    <option value="" disabled>
                      {availableBooksForSale.length
                        ? "Выберите книгу"
                        : "Нет книг для продажи"}
                    </option>
                    {availableBooksForSale.map((book) => (
                      <option key={book.id} value={book.id}>
                        {book.title} — остаток {bookQuantityFormatter.format(book.quantity)} шт.
                      </option>
                    ))}
                  </select>
                </label>
                ...
                <label className="flex flex-col gap-2">
                  ...
                  <input
                    type="number"
                    name="soldBookQuantity"
                    min="1"
                    step="1"
                    max={selectedBookForSale ? selectedBookForSale.quantity : undefined}
                    placeholder={selectedBookForSale ? `Максимум ${bookQuantityFormatter.format(selectedBookForSale.quantity)}` : "Например, 5"}
                    required
                    disabled={!availableBooksForSale.length || isBookSaleSubmitting}
                    className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-base text-slate-900 shadow-sm transition focus:border-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-400 disabled:cursor-not-allowed disabled:bg-slate-100 dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:disabled:border-slate-700/60 dark:disabled:bg-slate-800/60"
                  />
                ...
                <label className="flex flex-col gap-2">
                  ...
                  <input
                    type="number"
                    name="soldBookSalePrice"
                    min="0"
                    step="0.01"
                    inputMode="decimal"
                    placeholder={
                      selectedBookForSale
                        ? `По умолчанию ${bookCurrencyFormatter.format(selectedBookForSale.salePrice)}`
                        : "Например, 300"
                    }
                    disabled={!availableBooksForSale.length || isBookSaleSubmitting}
                    className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-base text-slate-900 shadow-sm transition focus:border-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-400 disabled:cursor-not-allowed disabled:bg-slate-100 dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:disabled:border-slate-700/60 dark:disabled:bg-slate-800/60"
              ...
              <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  Поле цены можно оставить пустым — возьмём цену из карточки книги.
                </p>
                <button
                  type="submit"
                  disabled={!availableBooksForSale.length || isBookSaleSubmitting}
                  className="inline-flex items-center justify-center rounded-xl bg-emerald-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-emerald-500 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:bg-emerald-600/70"
                >
                  {isBookSaleSubmitting ? "Сохраняем..." : "Зафиксировать продажу"}
                </button>
              </div>
            </form>

            <section className="rounded-3xl border border-dashed border-slate-300 bg-slate-50/60 p-6 text-slate-600 shadow-sm dark:border-slate-700 dark:bg-slate-900/60 dark:text-slate-300">
              {bookItems.length ? (
                <div className="space-y-4">
                  <header className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100">Добавленные книги</h3>
                      <p className="text-sm text-slate-500 dark:text-slate-400">
                        Обновите записи при необходимости — список хранится локально.
                      </p>
                    </div>
                    <span className="text-xs uppercase tracking-[0.24em] text-slate-500 dark:text-slate-400">
                      Всего: {bookQuantityFormatter.format(totalBooksCount)}
                    </span>
                  </header>
                  ...
                  <ul className="space-y-3">
                    {bookItems.map((item) => (
                      <li
                        key={item.id}
                        className="rounded-2xl border border-slate-200 bg-white px-4 py-4 shadow-sm transition hover:border-slate-300 hover:shadow dark:border-slate-700 dark:bg-slate-900/70 dark:hover:border-slate-600"
                      >
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                          <div>
                            <h4 className="text-base font-semibold text-slate-900 dark:text-white">{item.title}</h4>
                            <p className="mt-1 text-sm text-slate-500 dark:text-slate-300">
                              Язык: {bookLanguageLabels[item.language] ?? item.language}
                            </p>
                            <p className="mt-1 text-xs text-slate-400 dark:text-slate-500">
                              Добавлено: {createdAtFormatter.format(new Date(item.createdAt))}
                            </p>
                          </div>
                          <dl className="grid gap-2 text-sm text-slate-600 dark:text-slate-300 sm:text-right">
                            <div>
                              <dt className="text-xs uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">Количество</dt>
                              <dd className="font-semibold">
                                {item.quantity > 0 ? (
                                  `${bookQuantityFormatter.format(item.quantity)} шт.`
                                ) : (
                                  <span className="text-rose-600 dark:text-rose-400">Нет в наличии</span>
                                )}
                              </dd>
                            </div>
                            <div>
                              <dt className="text-xs uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">Закупка</dt>
                              <dd className="font-semibold">
                                {bookCurrencyFormatter.format(item.purchasePrice)}
                              </dd>
                            </div>
                            <div>
                              <dt className="text-xs uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">Реализация</dt>
                              <dd className="font-semibold">
                                {bookCurrencyFormatter.format(item.salePrice)}
                              </dd>
                            </div>
                            <div>
                              <dt className="text-xs uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">Долг</dt>
                              <dd className="font-semibold text-amber-600 dark:text-amber-400">
                                {bookCurrencyFormatter.format(item.debt)}
                              </dd>
                            </div>
                          </dl>
                        </div>
                ...
                <div className="flex flex-col items-center justify-center gap-3 text-center">
                  <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-100">Список книг пуст</h3>
                  <p className="text-sm text-slate-500 dark:text-slate-400">
                    Добавьте первую запись, чтобы вести учёт литературы и расчётов.
                  </p>
                </div>
              )}
            </section>

            <section className="rounded-3xl border border-slate-200 bg-white/70 p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900/70">
              {soldBookItems.length ? (
                <div className="space-y-4">
                  <header className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100">Проданные книги</h3>
                      <p className="text-sm text-slate-500 dark:text-slate-400">
                        История продаж сохраняется локально на этом устройстве.
                      </p>
                    </div>
                  ...
                  <ul className="space-y-3">
                    {soldBookItems.map((record) => (
                      <li
                        key={record.id}
                        className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 shadow-sm transition hover:border-slate-300 hover:shadow dark:border-slate-700 dark:bg-slate-900/70 dark:hover:border-slate-600"
                      >
                        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                          <div>
                            <h4 className="text-base font-semibold text-slate-900 dark:text-white">{record.title}</h4>
                            <p className="mt-1 text-sm text-slate-500 dark:text-slate-300">
                              Язык: {bookLanguageLabels[record.language] ?? record.language}
                            </p>
                            <p className="mt-1 text-xs text-slate-400 dark:text-slate-500">
                              Продано: {createdAtFormatter.format(new Date(record.soldAt))}
                            </p>
                          </div>
                          <dl className="grid gap-2 text-sm text-slate-600 dark:text-slate-300 sm:text-right">
                            <div>
                              <dt className="text-xs uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">Количество</dt>
                              <dd className="font-semibold">
                                {bookQuantityFormatter.format(record.quantity)} шт.
                              </dd>
                            </div>
                            <div>
                              <dt className="text-xs uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">Цена</dt>
                              <dd className="font-semibold">
                                {bookCurrencyFormatter.format(record.salePrice)}
                              </dd>
                            </div>
                            <div>
                              <dt className="text-xs uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">Выручка</dt>
                              <dd className="font-semibold text-emerald-600 dark:text-emerald-400">
                                {bookCurrencyFormatter.format(record.total)}
                              </dd>
                            </div>
                          </dl>
                        </div>
                ...
                <div className="flex flex-col items-center justify-center gap-3 text-center">
                  <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-100">Продаж пока нет</h3>
                  <p className="text-sm text-slate-500 dark:text-slate-400">
                    Зафиксируйте первую продажу, чтобы увидеть её в истории.
                  </p>
                </div>
              )}
            </section>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white/70 p-10 text-center text-slate-500 shadow-sm dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-300">
            <p className="text-sm">
              Выберите вкладку, чтобы продолжить работу с инвентарём или книгами.
            </p>
          </div>
        )}
      </div>
