...
const WarehousePage = () => {
  ...
  const handleBookSubmit = useCallback(
    (event: FormEvent<HTMLFormElement>) => {
      ...
      const language = String(formData.get("bookLanguage") ?? "").trim();
      ...
      if (!language) {
        setBookFormError("Укажите язык издания");
        return;
      }
      ...
      const normalizeMoney = (value: number) =>
        Number.isFinite(value) && value >= 0 ? Math.round(value * 100) / 100 : NaN;
                      ...
                      >
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                          <div>
                            ...
                            <p className="mt-1 text-sm text-slate-500 dark:text-slate-300">Язык: {item.language}</p>
